...
2. 玩家相撞：
   1. 兩隻魷魚相撞時，如果一方等級較高，則等級高者加 `3` 分，等級低者扣 `3` 分。
   2. 如果兩方等級相同，則雙方皆扣 `2` 分。
   3. 受傷那一方，會麻痺 `12` frame，過程中隨機向一個方向移動，移動期間，不會受到攻擊，也不會吃任何食物。
   4. 受傷那一方，麻痺效果解除後，將會有 `75` frame 的無敵時間，不會再受到對手的攻擊，但可以吃食物。
...
3. 若兩人同時通關，分數較高者勝。
4. 若兩人同時通關分數相同，除了將會延長遊戲時間 `300` frame、增加垃圾數量，還會額外提高 `目標分數` `50` 分。
